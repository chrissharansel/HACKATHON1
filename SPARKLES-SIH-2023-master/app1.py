...
def predict():
    try:
        ...
        if insta_data:
            ...
            behavior_messages = {reason: get_behavior_message(reason) for reason in selected_reasons}

            # If 'Other Reasons' is selected, include it in the result
            if 'others' in selected_reasons:
                behavior_messages['others'] = get_behavior_message('others')
...
def get_behavior_message(reason):
    ...
    behavior_mapping = {
        'spamMessages': 'Less suspicious behavior.',
        'linkSpamming': 'Abnormal behavior.',
        'identityFraud': 'Suspicious behavior.',
        'contenttheft': 'Suspicious behavior.',
        'jobscam': 'Abnormal behavior.',
        'fakenews': 'Suspicious behavior.',
        'adfraud': 'Abnormal behavior.',
        'bankAccount': 'Very highly suspicious behavior.',
        'otp': 'Very highly suspicious behavior.',
        'others': 'Suspicious behavior.',
    }
    return behavior_mapping.get(reason, 'Normal behavior')
